...
namespace AwariTest
{
    ...
    public class AwariGameModelTest
    {
        ...
        public void AwariGameStepAgain()
        {
            model.BinNumber = 4;
            model.NewGame(4);
            ...
            model.StepGame(4);
            Assert.AreEqual(Player.RedPlayer, model.CurrentPlayer);
            Assert.AreEqual(2, model.MoveCount);

            model.StepGame(1);
            Assert.AreEqual(Player.RedPlayer, model.CurrentPlayer);
            Assert.AreEqual(3, model.MoveCount);
        }

        [TestMethod]

        public void AwariGameStepOpposite()
        {
            model.BinNumber = 8;
            model.NewGame(8);
            ...
            model.StepGame(8);
            Assert.AreEqual(Player.RedPlayer, model.CurrentPlayer);
            Assert.AreEqual(4, model.MoveCount);

            model.StepGame(4);
            Assert.AreEqual(Player.RedPlayer, model.CurrentPlayer);
            Assert.AreEqual(5, model.MoveCount);

            model.StepGame(1);
            Assert.AreEqual(Player.BluePlayer, model.CurrentPlayer);
            Assert.AreEqual(6, model.MoveCount);
            Assert.AreEqual(14, model.Table[model.BinNumber / 2]);
        }
